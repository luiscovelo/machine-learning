...
PATH_SAVE_MODEL = "files/saved_model/LinearSVC.sav"


class ClassifierLinearSVC:
    ...
    def predict(self, pipe):
        ...
        predictProbab = zip(pipe.classes_, pipe.decision_function([words])[0])
        predictProbab = sorted(predictProbab, key=lambda tup: tup[1], reverse=True)

        msg = "Melhor categoria usando LinearSVC: " + predictCategory[0] + " com {:.1%} de acerto. ".format(predictProbab[0][1])
